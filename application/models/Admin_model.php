...
class Admin_model extends CI_Model {
    ...
    public function generate_qucode($id, $type)
    {
        ...
        $params['data'] = base_url().'render/'.$type.'/view/'.md5($id);
        $params['level'] = 'H';
        $params['size'] = 8;
    ...
    // select by function
    function get_by_result($id, $table)
    {
        $this->db->select();
        $this->db->from($table);
        $this->db->where('id', $id);
    ...
    function get_clone_invoice_item_by_result($id, $table)
    {
        $this->db->select();
        $this->db->from($table);
    ...
    //get invoice_number
    public function get_auto_invoice_number($user_id, $type, $recurring, $invoice_id)
    {
        $this->db->select('*');
    ...
    {
        $this->db->select('p.*, k.id as package_id, k.name as package');
    ...
    function add_recurring_payments ($table, $primary_field, $primary_val) 
    {
       ...
       $query = $this->db->get($table);
        ...
        $recurring_data = array(
            ...
            'number' => get_auto_invoice_number($invoice->user_id, 1, 1, $insert_id),
