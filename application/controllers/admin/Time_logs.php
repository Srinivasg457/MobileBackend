  ...
  public function checkExistingTimelog() 
{
    try {
        ...
        if (!empty($session_id)) {
            ...
            if ($query->num_rows() > 0) {
                ...
                return $this->output
                    ...
                    ->set_output(json_encode([
                        'status' => true,
                        'has_incomplete_session' => empty($result['end_time']) || $result['end_time'] <= $result['start_time'],
                        'data' => $result
                    ]));
            }
        }

        // Check for incomplete logs (end_time is NULL or <= start_time)
        $this->db->select('log_id, employee_id, user_id, log_date, start_time, 
                         end_time, total_active_time, total_idle_time, 
                         created_at, updated_at, session_id');
        $this->db->from('time_logs');
        ...
        $this->db->where('employee_id', $employee_id);
        $this->db->where('DATE(log_date)', $date);
        $this->db->where('(end_time IS NULL OR end_time <= start_time)');
        ...
        // If no incomplete session found, return any existing log for the date
        $this->db->select('log_id, employee_id, user_id, log_date, start_time, 
                         end_time, total_active_time, total_idle_time, 
                         created_at, updated_at, session_id');
        $this->db->from('time_logs');
 ...
 public function update_timelog()
{
    ...
    try {
        ...
        // Get existing log by session_id to ensure we update the correct session
        $this->db->where('employee_id', $headers['employee_id']);
        $this->db->where('user_id', $headers['user_id']);
        $this->db->where('session_id', $headers['session_id']);
        ...
        if (!$existing) {
            return $this->output
                ...
                ->set_output(json_encode([
                    "status" => "error",
                    "message" => "Time log session not found for update"
                ]));
        }
    ...
    // Log success
    log_message('info', "Time log updated for employee {$headers['employee_id']} session {$headers['session_id']}");
