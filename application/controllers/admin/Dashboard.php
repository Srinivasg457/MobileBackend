...
class Dashboard extends Home_Controller {
    ...
    {   
        ...
        if($_POST){
            ...
            if (is_employee()) {
                ...
                $user_id = $this->session->userdata('employee_org_id'); // assuming this is the user_id
    ...
    private function get_weekly_report_data($organization_id, $employee_id)
    {
    ...
    private function get_user_oldest_activity_date($organization_id, $employee_id)
    {
    ...
    private function get_this_week_inactive_time_data($user_id, $employee_id)
    {
        $employee_id = $this->session->userdata('employee_id');
    ...
    private function get_this_week_avarage_time_data($user_id, $employee_id)
    {
        if (empty($employee_id) || empty($user_id) || !is_numeric($employee_id) || !is_numeric($user_id)) {
            return [
                'status' => 'error',
    ...
    private function compare_weekly_average_active_time($user_id, $employee_id)
    {
        if (empty($employee_id) || empty($user_id) || !is_numeric($employee_id) || !is_numeric($user_id)) {
            return [];
        }
    ...
    public function get_this_week_inactive_time($user_id, $employee_id)
    {
        try {
    ...
    public function get_this_week_target_time_data($user_id, $employee_id)
    {
        if (empty($employee_id) || empty($user_id) || !is_numeric($employee_id) || !is_numeric($user_id)) {
            return [];
        }
    ...
    private function get_yesterday_comparison_summary($user_id, $employee_id)
    {
        if (empty($employee_id) || empty($user_id) || !is_numeric($employee_id) || !is_numeric($user_id)) {
            return null;
        }
    ...
    public function get_last_week_total_active_hours($user_id, $employee_id)
    {
        try {
            if (empty($employee_id) || empty($user_id) || !is_numeric($employee_id) || !is_numeric($user_id)) {
                return null;
